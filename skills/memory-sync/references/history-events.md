...
- Event body is the source of truth: `.wingman/memory/history/events/YYYY/MM/YYYY-MM-DD-<event-slug>.md`.
...
1. Create `.wingman/memory/history/`, `history/events/YYYY/MM/`, `history/domains/`, and `history/months/` if needed.
...
7. Include `Promoted Truths` links when domain truth or project ADR was written. Use `None` when no current truth was promoted.
8. Do not rewrite event bodies just because a projection index changes.
...
- Event bodies remain under `history/events/YYYY/MM/` and are not copied into domain or month directories.
