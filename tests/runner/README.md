...
```bash
node tests/runner/run-skill-eval.mjs data-contracts --case ALIGN-002 --dry-run
```
...
`data-contracts` 现在用这层检查 `ALIGN-002` 的语义漂移和 `ALIGN-004` 的伪造缺失字段。新增 skill 不需要写 checker；只有那些“明显不能出现”的代码形态值得加这种规则。
