...
Ordinary memory-load cases can be prepared by the memory runner:
...
The runner creates `.eval-runs/memory-load/<run-id>/<case-id>/` directories with isolated `workspace/`, `prompt.md`, and `evidence-template.json` files.

Mini comparison cases such as `MEMLOAD-008A/B` are not supported by this runner yet and must be run manually until a comparison runner exists.
