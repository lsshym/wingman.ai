...
wingman.ai/
  ...
  .agents/plugins/marketplace.json # Codex Git-backed marketplace entry
```

本项目写作规范：

1. 新建或修改 Wingman skill，优先编辑根目录 `skills/<skill-name>/SKILL.md`。
2. Codex 分发已迁移为 Git-backed marketplace。默认只维护根目录 `skills/`；`plugins/wingman/` payload 副本不再是默认发布路径。
3. 不要重新引入 `plugins/wingman/skills/` 副本，除非明确是在独立 marketplace payload 仓或回滚方案中工作。
4. skill 文件夹名、`SKILL.md` frontmatter 里的 `name` 必须一致。
5. 面向 Codex 官方本地发现时，Codex 还支持 `.agents/skills`、`$HOME/.agents/skills`、`/etc/codex/skills` 等位置；但本仓库作为插件项目时，根目录 plugin root 是主要分发路径。
...
9. 如果要分发到 Codex 插件，已确认根目录 plugin root、Git-backed marketplace entry 和发布检查通过。
