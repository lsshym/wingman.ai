...
这条命令会把当前源码同步到 `plugins/wingman/`，供 Codex marketplace 读取。
...
该脚本会把当前 Wingman 的 Codex payload 同步到目标仓库的 `plugins/wingman/`，并排除 `.agents/`、`.claude-plugin/`、`.cursor-plugin/`、`hooks/`、`scripts/` 等开发/其他平台文件。默认 npm scripts 不暴露外部 checkout 同步入口，日常发布只需要 `npm run prepare:codex-local`。
